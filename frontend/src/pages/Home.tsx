...
// ==========================================
// 1. DEFINITIONS & INTERFACES (Định nghĩa kiểu dữ liệu)
// ==========================================
...
// Component StatCard thiết kế riêng cho Staff (High-tech)
    ...
    >
        <CardContent sx={{ p: 3, position: "relative", zIndex: 1 }}>
            <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                ...
                <Box sx={{
                    p: 1.5, borderRadius: 3, bgcolor: "rgba(255,255,255,0.25)", backdropFilter: "blur(10px)",
                }}>
                    {icon}
                </Box>
            </Box>
        ...
        <Box sx={{
            position: "absolute", right: -15, bottom: -15, opacity: 0.15, transform: "rotate(-15deg)", fontSize: "100px",
        }}>
            {icon}
        </Box>
    </Card>
...
// --- STAFF DASHBOARD (Giao diện mới High-tech) ---
const StaffDashboard = ({ user, roleConfig, navigate, onLogout, stats }: StaffDashboardProps) => (
    <Box sx={{
        minHeight: "100vh", bgcolor: "#f8fafc", pb: 10,
        fontFamily: "'Inter', sans-serif", "& *": { fontFamily: "'Inter', sans-serif !important" },
    }}>
        <Header user={user} roleConfig={roleConfig} onLogout={onLogout} />

        {/* Banner tím High-tech */}
        <Box sx={{
            minHeight: 240, background: "linear-gradient(135deg, #6a1b9a 0%, #9c27b0 100%)",
            pt: 4, pb: 2, mb: 4,
        }}>
            <Container maxWidth="xl">
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12} sm={6} md={3}>
                        <StaffStatCard title="Môn Học" value={stats?.totalSubjects || 0} icon={<SourceIcon fontSize="large" />} color="#f59e0b" />
                    </Grid>
                    <Grid item xs={12} sm={6} md={3}>
                        <StaffStatCard title="Lớp Học" value={stats?.totalClasses || 0} icon={<CastForEducationIcon fontSize="large" />} color="#3b82f6" />
                    </Grid>
                    <Grid item xs={12} sm={6} md={3}>
                        <StaffStatCard title="Sinh Viên" value={stats?.totalUsers || 0} icon={<GroupAddIcon fontSize="large" />} color="#10b981" />
                    </Grid>
                    <Grid item xs={12} sm={6} md={3}>
                        <StaffStatCard title="Đề Tài" value={stats?.totalProjects || 0} icon={<RocketLaunchIcon fontSize="large" />} color="#8b5cf6" />
                    </Grid>
                </Grid>
            </Container>
        </Box>
            ...
            <Grid container spacing={4}>
                ...
                <Grid item xs={12} md={8}>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={6}>
                            <MenuCard title="Quản Lý Môn Học" desc="Import và cấu hình Syllabus hệ thống" icon={<AssignmentIcon />} color="#9c27b0" onClick={() => navigate('/admin/subjects')} />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                            <MenuCard title="Quản Lý Lớp Học" desc="Mở lớp, chia nhóm & nhập dữ liệu" icon={<SchoolIcon />} color="#9c27b0" onClick={() => navigate('/admin/classes')} />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                            <MenuCard title="Quản Lý Tài Khoản" desc="Dữ liệu Giảng viên & Sinh viên" icon={<PersonAddAlt1Icon />} color="#9c27b0" onClick={() => navigate('/staff/users')} />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                ...
                <Grid item xs={12} md={4}>
                    ...
                    <Paper sx={{ p: 4, borderRadius: 5, border: "1px solid #e2e8f0", textAlign: "center", bgcolor: "white" }}>
                        ...
                        <Button fullWidth variant="contained" sx={{ borderRadius: 3, py: 1.5, bgcolor: "#ed6c02", fontWeight: 800, "&:hover": { bgcolor: "#e65100" } }} onClick={() => navigate('/admin/import')}>
                            Bắt đầu Import
                        </Button>
                    </Paper>
                </Grid>
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#f1f8e9' }}>
        ...
        <Container maxWidth="xl" sx={{ py: 4 }}>
            <Box mb={5}>
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12} sm={6} md={4}><StatCard title="Lớp Đang Học" value={myClassCount || 0} icon={<ClassIcon fontSize="large" />} color="#2e7d32" /></Grid>
...
const Home = () => {
    ...
    const [stats, setStats] = useState({
        totalUsers: 0, activeUsers: 0, totalClasses: 0, totalSubjects: 0, totalProjects: 0
    });
